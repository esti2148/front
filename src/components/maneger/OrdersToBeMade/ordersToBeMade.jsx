...
const OrdersToBeMade = () => {
  ...
  // function handleCustomerNameChange(id) {
  //   debugger
  //   let name = dispatch(getByIdCustomerThunk(id))
  //   return name.instituteName
  // }
  ...
  const filteredOrders = ord.filter((order, index) => {
    ...
    //  order?.instituteId.includes(searchTerm) || 
    const matchesSearch = order?.itemOreders?.some(item => item?.productName.includes(searchTerm))
        ...
        {filteredOrders?.length === 0 ? (
          ...
          <div className="orders-grid">
            {filteredOrders?.map(order => (
              order.supplyDate && new Date(order?.supplyDate) > (new Date()) && (
